...
class MainProgram
{
    ...
    function list(){
        $rows = DB::table($this->tbl.' as m')->join('department as d','m.department_id','=','d.id')->selectRaw('m.id,m.name,d.name as department,m.updated_at')->get();
        foreach($rows as $row){
    ...
    function details($id){
        $row = DB::table($this->tbl)->where('id',$id)->selectRaw('id,name,department_id,updated_at')->first();
